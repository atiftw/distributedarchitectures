...
class SimpleSocketServer(val brokerId: Int,
                         val host: String,
                         val port: Int,
                         val kafkaApis: AwesomeKafkaApis
                        ) extends Logging {
   ...
   */
  def startup() {
    listener = new TcpListener(InetAddressAndPort.create(host, port), this)
    listener.start()
...
class TcpListener(localEp: InetAddressAndPort, socketServer: SimpleSocketServer) extends Thread with Logging {
