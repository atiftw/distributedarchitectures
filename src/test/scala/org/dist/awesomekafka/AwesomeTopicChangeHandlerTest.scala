...
class AwesomeTopicChangeHandlerTest extends ZookeeperTestHarness {
  ...
  test("should register for topic change and get replica assignments") {
    ...
    val createTopicCommand = new CreateAwesomeTopicCommand(zookeeperClient)
    val testContext = new TestContext
    val topicChangeListener = new AwesomeTopicChangeHandler(zookeeperClient, testContext.leaderAndIsr)
